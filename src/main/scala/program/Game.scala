...
import domain.{ Card, Deck, Defuse, GameExit, GameType }
...
object Game {
  def play[F[_]: Sync](
      gameType: GameType,
      console: ConsoleAlg
  ): EitherT[F, GameExit, (Card, Deck)] = {
    def playGame(deck: Deck, playerCard: Option[Defuse.type]): EitherT[F, GameExit, (Card, Deck)] =
      for {
        _ <- EitherT.right(
